...
type NavTab = "info" | "media" | "tech" | "timeline" | "features" | "team";

const Navs = ({ initialData, onClose = () => {} }:NavsProps) => {
  const [activeNav, setActiveNav] = useState<NavTab>("info");
      ...
      <form onSubmit={onSubmit}>
        ...
        <nav className="mb-6 w-full rounded-md bg-gray-100 p-1">
          <ul className="flex items-center justify-between">
            {(["info", "media", "tech", "timeline", "features", "team"] as NavTab[]).map(
              (tab) => (
                <li
                  key={tab}
