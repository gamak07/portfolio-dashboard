...
const Header = () => {
  ...
  const [activeList, setActiveList] = useState<string>("grid");
  const handleList = (list:string) => {
    setActiveList(list);
  };

  const [showAddNewProjectModal, setAddNewProjectModal] = useState<boolean>(false);
  ...
  return (
    <>
      <div className="gap-d mb-6 flex flex-col justify-between md:flex-row md:items-center">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative" onClick={handleDropdown}>
            <Button className="flex cursor-pointer items-center rounded-lg border border-gray-300 bg-white px-4 py-2 text-gray-700 hover:bg-gray-50 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200 dark:hover:bg-gray-700">
              <span>Category:</span>
          ...
          <div className="flex items-center rounded-lg border border-gray-300 bg-white dark:border-gray-700 dark:bg-gray-700">
            <Button
              className={`cursor-pointer rounded-l-lg px-3 py-2 ${activeList === "grid" ? "bg-gray-100 text-indigo-600 dark:bg-gray-700 dark:text-indigo-400" : "text-gray-600"}`}
              onClick={() => handleList("grid")}
            >
              <FaThLarge className="" />
            </Button>
            <Button
              className={`cursor-pointer rounded-r-lg px-3 py-2 ${activeList === "list" ? "bg-gray-100 text-indigo-600 dark:bg-gray-700 dark:text-indigo-400" : "text-gray-600"}`}
      ...
      {showAddNewProjectModal && <AddNewProjects setAddNewProjectModal={()=>setAddNewProjectModal(false)} />}
    </>
  );
};
